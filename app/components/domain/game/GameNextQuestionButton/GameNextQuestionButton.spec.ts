...
describe("GameNextQuestionButton Component", () => {
  ...
  async function mountButton(options: MountSuspendedOptions<typeof GameNextQuestionButton> = {}): Promise<VueWrapper> {
    return mountSuspended(GameNextQuestionButton, { shallow: false, ...options });
  }
  ...
  it("should not apply the themed class when disabled.", async() => {
    ...
    expect(wrapper.find("button").classes()).not.toContain("game-question-navigation-button--themed");
  });
